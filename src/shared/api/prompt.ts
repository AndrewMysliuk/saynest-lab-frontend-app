...
import { IIeltsPromptFilters, IModuleFilters, IModuleScenarioEntity, IPromptFilters, IPromptScenarioEntity } from "../types"
...
export const listIeltsScenariosHandler = async (query?: IIeltsPromptFilters): Promise<IPromptScenarioEntity[]> => {
  try {
    const response: AxiosResponse = await axios({
      url: "/api/prompts-library/ielts-scenarios",
      method: "GET",
      params: query,
    })
