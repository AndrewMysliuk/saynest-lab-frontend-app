      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
        <h3 class="text-xl font-semibold text-gray-800">
          {{
            currentIeltsPart === SessionIeltsPartEnum.PART_1
              ? scenario.model_behavior.ielts_scenario?.part1.topics?.map((topic) => topic.title).join(", ")
              : currentIeltsPart === SessionIeltsPartEnum.PART_2
                ...
                : currentIeltsPart === SessionIeltsPartEnum.PART_3
                  ? scenario.model_behavior.ielts_scenario?.part3.topics?.map((topic) => topic.title).join(", ")
                  : scenario.title
          }}
        </h3>
        <div class="flex flex-wrap gap-2 text-xs items-center">
          ...
          <span v-if="getCurrentUserProgress[scenario.name]" class="bg-green-100 text-green-700 font-semibold px-2 py-1 rounded-full">
            Completed {{ getPartProgress(getCurrentUserProgress, scenario.name, currentIeltsPart ? currentIeltsPart : "FULL_SCENARIO") }}x
          </span>
          <span v-else class="bg-gray-100 text-gray-500 font-semibold px-2 py-1 rounded-full"> Not completed </span>
        </div>
      </div>

      <p v-if="currentIeltsPart === null" class="text-sm text-text-muted mb-5">{{ scenario.description }}</p>
