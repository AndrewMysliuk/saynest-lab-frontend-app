  ...
  <div class="py-10 px-6" v-if="getCurrentReview">
    ...
    <div class="mt-8 bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
      <nav class="flex gap-2 relative overflow-x-auto no-scrollbar after:absolute after:bottom-0 after:inset-x-0 after:border-b after:border-stone-200 sm:flex-nowrap">
        ...
        >
          {{ t("history.details.tabs.transcript") }}
        </button>
        ...
        <div class="space-y-6">
          <div class="flex flex-wrap items-center gap-3 text-gray-600">
            ...
            <span v-if="getCurrentReview.user_ielts_mark" class="px-3 py-1 bg-blue-100 text-blue-800 text-base font-semibold rounded-md">
              {{ t("history.details.summary.badges.ielts") }}: {{ getCurrentReview.user_ielts_mark.toFixed(1).replace(".", ",") }}
            </span>
            ...
            <button @click="generatePublicLink" class="ml-auto inline-flex items-center gap-2 px-3 py-1.5 text-base font-medium text-white bg-[#4F46E5] hover:bg-[#4338CA] rounded-md transition">
              ...
              {{ t("history.details.summary.share") }}
            </button>
          </div>
          ...
          <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-500 mt-4">
            ...
            <p>
              <i class="fas fa-comments mr-1"></i> <b>{{ t("history.details.summary.meta.messages") }}:</b> {{ getCurrentReview.history.messages.length - 2 }}
            </p>
            <p>
              <i class="fas fa-user mr-1"></i> <b>{{ t("history.details.summary.meta.you") }}:</b> {{ getCurrentReview.history.user_utterances_count - 1 }}
            </p>
            <p>
              <i class="fas fa-robot mr-1"></i> <b>{{ t("history.details.summary.meta.ai") }}:</b> {{ getCurrentReview.history.model_utterances_count }}
            </p>
          </div>
          ...
          <div v-if="getCurrentReview.band_breakdown" class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-md">
            ...
            <ul class="list-disc list-inside text-sm text-blue-900 space-y-1">
              <li>
                <strong>{{ t("history.details.summary.band.fluency") }}:</strong> {{ getCurrentReview.band_breakdown.fluency }}
              </li>
              <li>
                <strong>{{ t("history.details.summary.band.lexical") }}:</strong> {{ getCurrentReview.band_breakdown.lexical }}
              </li>
              <li>
                <strong>{{ t("history.details.summary.band.grammar") }}:</strong> {{ getCurrentReview.band_breakdown.grammar }}
              </li>
            </ul>
          </div>
          ...
          <div v-if="getCurrentReview.part1" class="bg-indigo-50 border-l-4 border-indigo-400 p-4 rounded-md">
            ...
            <p class="text-sm text-indigo-900 mb-2">{{ getCurrentReview.part1.summary }}</p>
            <div v-if="getCurrentReview.part1.highlights?.length">
              ...
              <ul class="list-disc list-inside text-sm text-indigo-900 space-y-1">
                <li v-for="(item, index) in getCurrentReview.part1.highlights" :key="index">{{ item }}</li>
              </ul>
            </div>
          </div>
          ...
          <div v-if="getCurrentReview.part2" class="bg-purple-50 border-l-4 border-purple-400 p-4 rounded-md">
            ...
            <p class="text-sm text-purple-900 mb-2">{{ getCurrentReview.part2.summary }}</p>
            <div v-if="getCurrentReview.part2.highlights?.length">
              ...
              <ul class="list-disc list-inside text-sm text-purple-900 space-y-1">
                <li v-for="(item, index) in getCurrentReview.part2.highlights" :key="index">{{ item }}</li>
              </ul>
            </div>
          </div>
          ...
          <div v-if="getCurrentReview.part3" class="bg-pink-50 border-l-4 border-pink-400 p-4 rounded-md">
            ...
            <p class="text-sm text-pink-900 mb-2">{{ getCurrentReview.part3.summary }}</p>
            <div v-if="getCurrentReview.part3.highlights?.length">
              ...
              <ul class="list-disc list-inside text-sm text-pink-900 space-y-1">
                <li v-for="(item, index) in getCurrentReview.part3.highlights" :key="index">{{ item }}</li>
              </ul>
            </div>
          </div>
      ...
      <div v-else-if="activeTab === CommunicationReviewHistoryEnum.DIALOGUE && getCurrentReview.history?.messages?.length" class="space-y-8">
        <h2 class="text-2xl font-bold text-text-base">{{ t("history.details.transcript.title") }}</h2>
          ...
          >
            <div class="flex items-center justify-between mb-2 text-sm text-gray-500">
              <div class="flex items-center gap-1">
                ...
                <span>{{ msg.role === "user" ? t("history.details.transcript.you") : t("history.details.transcript.ai") }}</span>
              </div>
      ...
      <div v-else-if="activeTab === CommunicationReviewHistoryEnum.TASKS" class="space-y-10">
        <h2 class="text-2xl font-bold text-text-base">{{ t("history.details.tasks.title") }}</h2>

        <div class="bg-gray-50 border border-gray-200 p-4 rounded-lg">
          <label for="task-select" class="block text-base font-medium text-gray-700 mb-2">
            {{ t("history.details.tasks.selectLabel") }}
          </label>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <button class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-md transition" @click="generateTask">
              {{ isLoading ? t("history.details.tasks.loading") : t("history.details.tasks.generate") }}
            </button>
          </div>
        </div>
