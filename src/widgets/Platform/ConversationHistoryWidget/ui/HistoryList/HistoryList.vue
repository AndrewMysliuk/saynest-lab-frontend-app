  ...
  <div class="py-10 px-6">
    ...
    <div v-if="getReviewsList.length" class="space-y-6">
      ...
      >
          ...
          <div class="lg:col-span-10">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-4">
              ...
              <span v-if="review.user_ielts_mark" class="px-2 py-0.5 bg-blue-100 text-blue-800 text-sm font-semibold rounded-md">
                {{ t("history.list.badges.ielts") }}: {{ review.user_ielts_mark.toFixed(1).replace(".", ",") }}
              </span>
            ...
            <div class="flex flex-wrap gap-4 text-xs text-gray-500">
              ...
              <p><i class="fas fa-comments mr-1"></i> {{ t("history.list.meta.messages") }}: {{ review.history.messages.length - 2 }}</p>
            </div>
          </div>

          <div class="lg:col-span-2 hidden lg:flex items-start gap-2 flex-col ml-auto">
            <button
              @click.prevent.stop="openDetails(review)"
              :title="t('history.list.actions.view')"
              class="size-8 inline-flex justify-center items-center text-gray-600 hover:text-blue-600 rounded-lg transition"
            >
              <i class="fas fa-eye text-base"></i>
            </button>
            <button
              @click.prevent.stop="deleteReview(review._id)"
              :title="t('history.list.actions.delete')"
              class="size-8 inline-flex justify-center items-center text-gray-600 hover:text-red-600 rounded-lg transition"
            >
        ...
        <div class="flex justify-end gap-4 mt-6 lg:hidden">
          <button
            @click.prevent.stop="openDetails(review)"
            :title="t('history.list.actions.view')"
            class="px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-200 transition"
          >
            {{ t("history.list.actions.view") }}
          </button>
          <button
            @click.prevent.stop="deleteReview(review._id)"
            :title="t('history.list.actions.delete')"
      ...
      <div v-if="isListLoading" class="text-center text-text-muted text-sm italic py-4">
        {{ t("history.list.loadingMore") }}
      </div>
    </div>

    <p v-else class="text-center text-text-muted text-sm italic">
