<template>
  <div>
    <div class="p-5 bg-white border border-stone-200 shadow rounded-xl">
      <h2 class="text-base font-semibold text-gray-900 mb-4">Latest Tasks</h2>
        ...
        >
          <div class="flex justify-between items-start gap-4">
            <div class="space-y-1">
              ...
              <p class="text-xs text-gray-500">Started: {{ formatDate(task.created_at) }}</p>
              <p v-if="task.is_completed" class="text-xs text-gray-500">Completed: {{ formatDate(task.completed_at) }}</p>
            </div>
